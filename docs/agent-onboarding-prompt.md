...
Drop the prompt below into any AI coding agent (Claude Code, Cursor,
Aider, etc.) when you want it to set up `bomtique` on **your** repo.
It assumes:

- `bomtique` is already installed locally and on the agent's `$PATH`
  (see [Install in the README](../README.md#install)).
- The agent should learn bomtique's CLI by running
  `bomtique --help` and each subcommand's `--help`, **not** by
  pulling docs from the network.
- The agent should analyse your repo before writing anything, and
...
Copy everything between the rules and paste it into your agent's
prompt window or system message.
...
## Adapting the prompt for a specific agent

The prompt above works as-is. Two optional refinements depending on
your agent and your repo:

- **Long projects**: if the repo has hundreds of subdirectories or
  the dependency tree is huge, append "Work on one
  primary/dependency at a time and pause for confirmation between
  each, rather than batching." after the operating constraints.
- **Strict review**: if you want every command run by hand, append
  "Show me each command before running it. I'll approve each one
  individually." Same place.

The prompt deliberately avoids tying itself to a particular agent
(Claude Code, Cursor, Aider, etc.); the surface it exercises is
just bomtique's CLI plus your repo. If your agent supports custom
project-level rules (e.g., a `CLAUDE.md` or `.cursorrules` file),
you may want to add the operating constraints there as well so they
